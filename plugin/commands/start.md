...
Launch Unreal Editor with the project in the current directory, managed by the ue5 server daemon. Logs are automatically captured and queryable via `/ue:logs`.
...
3. **Report success to user**
   ...
   - Remind that `/ue:logs` can be used to view captured logs
   - Remind that `/ue:stop` stops the managed instance

## Important Notes

- The editor is managed by the daemon -- use `/ue:stop` to stop it (not pkill)
- Logs are captured automatically and queryable via `/ue:logs`
